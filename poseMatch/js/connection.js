...
var Connection = function (me, peer, call) {
    ...
    console.log(
        `New connection name is ${this.pcName} (caller: ${this.caller}, callee: ${this.callee})`
    );
...
Connection.prototype.initDB = async function (pcName) {
    ...
    await this.pcCollectionRef.set({
        caller: this.caller,
        callee: this.callee,
    });

    this.callerCandidatesCollection = this.pcCollectionRef.collection(
        'callerCandidates'
    );
    this.calleeCandidatesCollection = this.pcCollectionRef.collection(
        'calleeCandidates'
    );
};
...
Connection.prototype.addRemoteStream = function (index, peer) {
    ...
    PoseMatch.getInstance()
        .getPEManager()
        .createVideoPose(video.id, peer, false);
};
...
Connection.prototype.startOffer = async function () {
    ...
    this.calleeUnsubscribe = this.calleeCandidatesCollection.onSnapshot(
        (snapshot) => {
            snapshot.docChanges().forEach(async (change) => {
                if (change.type == 'added') {
                    let data = change.doc.data();
                    console.log(
                        `caller addIceCandidate(calleeCandidates) ${JSON.stringify(
                            data
                        )}`
                    );
...
Connection.prototype.startAnswer = async function () {
    ...
    this.callerUnsubscribe = this.callerCandidatesCollection.onSnapshot(
        (snapshot) => {
            snapshot.docChanges().forEach(async (change) => {
                if (change.type === 'added') {
                    let data = change.doc.data();
                    console.log(
                        `callee addIceCandidate(callerCandidates) ${JSON.stringify(
                            data
                        )}`
                    );
...
Connection.prototype.registerPeerConnectionListeners = function () {
    ...
    this.peerConnection.addEventListener('connectionstatechange', () => {
        console.log(
            `[${this.pcName}] Connection state change: ${this.peerConnection.connectionState}`
        );
        if (this.peerConnection.connectionState == 'connected') {
            this.stateListeners.forEach((listener) => {
                listener(
                    'connected',
                    this.remoteCanvas.id,
                    this.remoteVideo.id,
                    this.peerConnection
                );
            });
    ...
    this.peerConnection.addEventListener('signalingstatechange', () => {
        console.log(
            `[${this.pcName}] Signaling state change: ${this.peerConnection.signalingState}`
        );
    });
...
Connection.prototype.hangup = async function () {
    if (this.remoteStream && this.remoteStream.getTracks()) {
        console.log(
            'Stop remote tracks. Size: ' + this.remoteStream.getTracks().length
        );
